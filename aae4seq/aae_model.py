...
class AAE():
    ...
    def load(self, prefix):
        self.encoder = keras.models.load_model(os.path.join(prefix, "encoder_protein_function.h5"))
        self.decoder = keras.models.load_model(os.path.join(prefix, "decoder_protein_function.h5"))
        self.discriminator = keras.models.load_model(os.path.join(prefix, "discriminator_protein_function.h5"))
        self.adversarial = keras.models.load_model(os.path.join(prefix, "adversarial_protein_function.h5"))
    ...
    def train(self, x_train, y_train, outputdir, 
        ...
        for epoch in range(train_steps):
            ...
            #print(g_loss)
            io.write_log(callback, ['d_loss', 'g_loss', "ae", "validity", "accuracy"], [d_loss[0], g_loss[0], g_loss[1], g_loss[2], g_loss[3]], epoch+1)
            # Plot the progress
            ...
            if (epoch + 1) % save_interval == 0:
                ...
                utils.rnd_sequence(self.decoder, idx2aa, z_samping,
                                       step = (epoch + 1),
...
def get_cmd():
    ...
    parser.add_argument("-l", action="store", dest="latentdim", type=int, default=100)
    parser.add_argument("-n", action="store", dest="niter", type=int, default=50000)
    parser.add_argument("-b", action="store", dest="batchsize", type=int, default=64)
    parser.add_argument("-r", action="store", dest="learning_rate", type=float, default=0.00001)
...
def main(params):
    ...
    if params.weightfile:
        weights = io.read_weights(params.weightfile)
        
    sequences, labels, _ = io.read_data(params.inputfile)
    x_train = utils.to_numeric(sequences, aa2idx)
    x_train = to_categorical(x_train, num_classes=len(utils.ORDER_LIST))
    print(x_train.shape)
